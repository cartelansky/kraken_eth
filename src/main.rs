...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if coins.is_empty() {
        println!("ETH çiftleri bulunamadı. API yanıtını kontrol edin.");
        return Ok(());
    }
    ...
    let mut file = File::create("kraken_eth_markets.txt")?;
    for coin in &coins {
        writeln!(file, "{}", coin)?;
    }

    println!("Kraken ETH spot piyasası coin listesi başarıyla oluşturuldu ve kaydedildi.");
    println!("Toplam {} adet ETH çifti bulundu.", coins.len());
    Ok(())
}
